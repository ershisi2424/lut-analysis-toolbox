...
- LUT 检查器：读取 `.cube` 文件，显示尺寸、范围、动态范围、对比度、曲线、色相分布、热力图、矢量示波器和基础色系总结。
- LUT 分析器：提供独立的可见导入入口，检查中性轴、平滑度、六个基础色系变化、饱和度响应和 IRE 响应。
- LUT 预览器：采用左侧 LUT 列表、中间转换参数、右侧固定预览布局，在内置图或用户图片上实时比较原图与 LUT 结果。
- 本地历史：自动保存最近 20 个 LUT 到本机 IndexedDB，相同文件按 SHA-256 去重，可在三个工具页面中恢复使用。
- 客户端界面：半透明液态玻璃工作区、动态高光、低动态效果回退，以及导入卡片内文件名和基本信息。
